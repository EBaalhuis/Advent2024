        ...
        let result = 
            ...
            let b = 
                ...
                Seq.map (fun pair ->
                    let x = fst pair
                    let y = snd pair
                    let c1 = if x > 0 && plants[x-1][y] = plants[x][y] then 0 else 1
                    let c2 = if x < max_x && plants[x+1][y] = plants[x][y] then 0 else 1
                    let c3 = if y > 0 && plants[x][y-1] = plants[x][y] then 0 else 1
                    let c4 = if y < max_y && plants[x][y+1] = plants[x][y] then 0 else 1 
